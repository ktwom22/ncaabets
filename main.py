...
def index():
    ...
    try:
        # User Agent helps avoid some 401/403 blocks
        ...
        if response.status_code == 200:
            ...
            print(f"SHEET ACCESS FAILED: Status {response.status_code}")
            df = pd.DataFrame()
    except Exception as e:
    ...
    # FIXED: Added seo object and stats to prevent Jinja2 UndefinedError
    return render_template('index.html',
                           games=all_games,
                           stats={"W": wins, "L": losses, "PCT": pct},
                           seo={"title": "Edge Engine Pro Dashboard"})
