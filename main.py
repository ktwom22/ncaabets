...
def auto_log_game(game_data, row_stats, existing_ids):
    """Appends game data using the EXACT column structure to prevent shifting."""
    ...
    try:
        new_row = [
            ...
            game_data['Matchup'],
            "PENDING", "0-0",  # Result and Score placeholders
...
def update_finished_results(adf, live_map):
    """Checks for PENDING games and settles them if finished."""
    ...
    for index, row in pending.iterrows():
        ...
        # Find this game in the live data
        game_info = next((v for k, v in live_map.items() if v['id'] == eid), None)

        if game_info and game_info['state'] == 'post':
            ...
            # Simple Cover Logic
            matchup = row['Matchup']
            is_home_pick = any(word in pick for word in matchup.split(" @ ")[1].upper().split())
            ...
            row_idx = index + 2  # +1 for header, +1 for 1-indexing
...
def index():
    ...
    if archive_sheet:
        try:
            ...
            if records:
                ...
                # Filter out those pesky shifting rows if they have no result
    ...
    # --- AUTO-UPDATE PENDING GAMES ---
    if not adf.empty:
        update_finished_results(adf, live_map)
    ...
    for _, row in df.iterrows():
        ...
        proj_left_away = round((avg_total / 2) - (our_margin / 2), 1)
        proj_right_home = round((avg_total / 2) + (our_margin / 2), 1)
        ...
        game_data = {
            ...
            'Pick': str(pick).upper(), 'Pick_Spread': p_spr, 'Left_Proj': proj_left_away, 'Right_Proj': proj_right_home,
